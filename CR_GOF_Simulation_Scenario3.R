...
#
# Simulation scenario 3: Models with behavioural effects and heterogeneity.
#
# We evaluate model GOFs (for 2 models only) using DHARMa. GOF residual plots 
# are assessed visually and we use the KS and Dispersion tests. 
#
...
# likelihood/Horvitz-Thompson estimator M_bh (GLM) using VGAM.
#
# A full description of these data and their analysis appears in the 
# Supplementary Materials (Web Section C3).
# ==============================================================================
...
alpha <- rep(0, tau)   # Time effect, this needs to be 0 for M_bh models.

rho_val <- -2         # Behave. effect (trap happt +ve, trap shy -ve).
rho <- rep(rho_val, N)

res <- matrix(NA, nsim, 4)

folder_name <- "Sim3_GOF"
...
for (ii in 1:nsim) {
  ...
  # Correctly specified: As above, but now accounts for behav. response. That is,
  # fit a M_bh GLM.
  ...
  # Fit model M_bh with VGAM (which uses the Huggins' conditional likelihood) 
  # to get the pop size.
  
  data_Mbh_VGLM <- data.frame(cbind(cap.hist, X_i))
  
  if (tau == 7) {
    colnames(data_Mbh_VGLM) <- c("y1", "y2", "y3", "y4", "y5", "y6", "y7", "X_i")
    
    M_bh <- vglm(cbind(y1, y2, y3, y4, y5, y6, y7) ~ X_i, 
                 posbernoulli.b, data = data_Mbh_VGLM, trace = FALSE) 
  }
  
  if (tau == 15) {
    colnames(data_Mbh_VGLM) <- c("y1", "y2", "y3", "y4", "y5", "y6", "y7", "y8",
                                 "y9", "y10", "y11", "y12", "y13", "y14", "y15", "X_i")
    
    M_bh <- vglm(cbind(y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, 
                       y13, y14, y15) ~ X_i, 
                 posbernoulli.b, data = data_Mbh_VGLM, trace = FALSE) 
  }
  
  N_hat3 <- M_bh@extra$N.hat
  
  # DHARMa plots: Side-by-side plots.
  
  figname <- file.path(folder_name, paste0("sim3_GOF_plots_dataset_", ii, ".png"))
  ...
  plotQQunif(res_grouped2, 
             main = paste("Dataset_", ii, ": Model 2 (Correctly specified, M_bh)"))
...
rownames(res_all) <- c("D", "PL-M_h", "PL-M_bh", "CL-M_bh")
res_all
